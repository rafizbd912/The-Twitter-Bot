...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        if (first == null || second == null) {
            throw new IllegalArgumentException("Both first and second words must be non-null");
        }
        chain.putIfAbsent(first, new ProbabilityDistribution<>());
    ...
    public void train(Iterator<String> sentence) {
        if (sentence == null) {
            throw new IllegalArgumentException("Sentence iterator cannot be null");
        }
        ...
        while (sentence.hasNext()) {
            String currentWord = sentence.next();
            addBigram(previousWord, currentWord);
            previousWord = currentWord;
        }

        addBigram(previousWord, END_TOKEN);
    }
    ...
    public boolean hasNext() {
        ...
        return !END_TOKEN.equals(currentWord);
    ...
    public String next() {
        ...
        if (!hasNext()) {
            throw new NoSuchElementException("No more words in the Markov chain");
        }

        String wordToReturn = currentWord;
        ProbabilityDistribution<String> dist = chain.get(currentWord);
        currentWord = dist == null ? END_TOKEN : dist.pick(ng);
        return wordToReturn;
    }
