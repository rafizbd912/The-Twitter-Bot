...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        if (csvColumn >= 0 && csvColumn < columns.length) {
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        List<String> splitSentences = tweetSplit(cleanedTweet);

        for (String sentence : splitSentences) {
            List<String> cleanWords = parseAndCleanSentence(sentence);
            if (!cleanWords.isEmpty()) {
                sentences.add(cleanWords);
            }
        }
    ...
    public static List<List<String>> csvDataToTrainingData(BufferedReader br, int tweetColumn) {
        ...
        for (String tweet : tweets) {
            List<List<String>> parsedTweets = parseAndCleanTweet(tweet);
            trainingData.addAll(parsedTweets);
        }
